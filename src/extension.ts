...
import { getGitShow, runPandiffAndGetContent } from "./content_F";
...
export async function activate(context: vscode.ExtensionContext) {
  const stylesFile: vscode.Uri = vscode.Uri.file(
    node_path.join(context.extensionPath, "styles", "style.css"),
  );
  outputChannel = vscode.window.createOutputChannel("Pandiff");
  let compareTwoFiles = vscode.commands.registerCommand(
    "pandiff-vscode.difs",
    async function () {
  ...
  let compareWithRevision = vscode.commands.registerCommand(
    "pandiff-vscode.compareRevision",
    async (...files: vscode.Uri[] | Array<any>) => {
      if (!(await isPandocInstalled())) return;
      ...
      let hashes: vscode.QuickPickItem[] | undefined =
        (await getFileRevisions(file))?.filter((line) => {
          if (line) {
            return true;
          } else return false;
        });
      ...
      if (!html) {
        vscode.window.showInformationMessage(
          "No Difference with Working Tree file",
        );
        unlinkTmpFile(tmp);
        return;
      }
      ...
      if (keepOutputFile) {
        await writeOutputFile(tmp, fileFullPath, outputFormat, [
          fileName,
          shortHash,
        ], stylizedHTML);
      }
  ...
  let rightClick = vscode.commands.registerCommand(
    "pandiff-vscode.rightClick",
  ...
  let compareTwoRevisions = vscode.commands.registerCommand(
    "pandiff-vscode.twoRevs",
    async () => {
      if (!(await isPandocInstalled())) return;
      ...
      let hashes: vscode.QuickPickItem[] | undefined = await getFileRevisions(
        file,
      );
      ...
      if (!html) {
        vscode.window.showInformationMessage(
          "No differences between Revisions",
        );
        unlinkTmpFile(tmp1);
        unlinkTmpFile(tmp2);
        return;
      ...
      if (keepOutputFile) {
        await writeOutputFile(tmp1, tmp2, outputFormat, [
          fileName,
          shortHash1,
          shortHash2,
        ], stylizedHTML);
  ...
  let editStyle = vscode.commands.registerCommand(
    "pandiff-vscode.editStyles",
    async () => {
