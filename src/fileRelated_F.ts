...
import { CleanOptions, LogResult, SimpleGit, simpleGit } from "simple-git";
...
export async function getFilesPath() {
  ...
  const allowedFiles = config.get("enabledFilesExtensions", [
    "docx",
    "odt",
    "txt",
    "md",
  ...
  let filesUri = await vscode.workspace.findFiles(
    spreadPatterns(patterizeAllowedFiles),
    spreadPatterns([
...
export function writeTmpFile(
  ...
  let tmp = changeFilename
    ? path.join(tmpFolder, "2_" + filename)
    : path.join(tmpFolder, "1_" + filename);
...
// foR = FileOrRevision
export function writeHTMLdirectly(
  labels: string,
  content: string,
  ext = "html",
) {
  ...
  if (!workspaceUri) return;
    ...
    const filePathInWorkspace = vscode.Uri.joinPath(
      workspaceUri,
      `_${labels}.${ext}`,
    );
    try {
...
export async function writeOutputFile(
  ...
  if (outputFormat === "HTML" && stylizedHTML) {
    ...
    const pandiffNewOutput = await runPandiffAndGetContent(
      path1,
      path2,
      "markdown",
      filePathInWorkspace.fsPath,
    );
